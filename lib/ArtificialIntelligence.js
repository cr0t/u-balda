...
export default class ArtificialIntelligence {
  ...
  findWord(cells, usedWords = []) {
    ...
    const availablePrefixes = this.vocabulary.availablePrefixes;
    let wordVariants = [];
    ...
    cells.forEach((cell, idx) => {
      ...
      if (cell === '' && nonEmptyNeighboursCount >= 1) {
        let wordsByCharacter = this._iterateCharacters(cells, idx, availablePrefixes);
  ...
  _iterateCharacters(cells, idx, availablePrefixes) {
    ...
    this.characters.forEach(c => {
      ...
      // const uniqueChains = chains.filter((value, index, self) => { return self.indexOf(value) === index; });
      // console.log('chains',  chains.length, uniqueChains.length);
      // console.log('chains', chains.length);
      chains.forEach((c) => {
        let len = c.length;
        if (!chainsByLength[len]) {
          chainsByLength[len] = [];
        }
        chainsByLength[len].push(c);
      });
      Object.keys(chainsByLength).forEach((k) => {
        console.log(k, chainsByLength[k].length);
      });
      wordsByCharacter[c] = this._checkChainsInVocabulary(chains);
     */
      const paths = new Graph(cellsCopy, idx, availablePrefixes).getPaths();
      // console.log(paths.length, paths);
      wordsByCharacter[c] = this._checkChainsInVocabulary(paths);
    });
