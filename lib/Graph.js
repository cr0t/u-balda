    ...
    paths.forEach((path) => {
      if (path.length > 2) {
        for (let i = 3; i <= path.length; i += 1) {
          ...
          // TODO: check in Trie if this prefix could make a word
          chains.push(chain);
        }
      }
    });

    console.log(paths[0], chains[0], chains[1], chains[2], chains[3], chains[4]);
    console.log(paths.length, chains.length);
